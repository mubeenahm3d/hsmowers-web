...
const serviceEmail = async (senderEmail, recipientEmail, message) => {
    ...
    const response = await resendClient.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          ...
          <p style="font-size: 16px; color: #333;">From: ${senderEmail}</p>
          <p style="font-size: 16px; color: #333;">Message: ${message}</p>
          <hr style="border: 0; height: 1px; background-color: #e0e0e0;">
          <p style="font-size: 12px; color: #999; text-align: center;">You’re receiving this email because ${senderEmail} requests you a servcie.</p>
        </div>
      `,
    });
